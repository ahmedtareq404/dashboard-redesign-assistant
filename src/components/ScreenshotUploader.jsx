...
export default function ScreenshotUploader() {
  ...
  function uploadFile() {
    ...
    xhr.onload = () => {
      ...
      setStatus("error");`r`n      try {`r`n        const response = JSON.parse(xhr.responseText);`r`n        setError(response.error || "Upload failed. Please try again.");`r`n      } catch {`r`n        setError("Upload failed. Please try again.");`r`n      }
    };
    xhr.onerror = () => {
      setStatus("error");
